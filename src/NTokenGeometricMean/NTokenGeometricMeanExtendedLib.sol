...
function findRootLiquidity(
    ...
    (uint256 rX, uint256 rY,, NTokenGeometricMeanParams memory params) =
        abi.decode(data, (uint256, uint256, int256, NTokenGeometricMeanParams));
...
function findRootLower(bytes memory data, uint256 v) pure returns (int256) {
    (
        ...
        data, (uint256, uint256, uint256, uint256, NTokenGeometricMeanParams)
    );
