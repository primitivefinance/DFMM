...
function findRootY(bytes memory data, uint256 ry) view returns (int256) {
...
function findRootX(bytes memory data, uint256 rx) view returns (int256) {
...
function computeNextLiquidity(
    ...
    if (computedInvariant < 0) {
        while (computedInvariant < 0) {
            lower = rx + 1000;
            console2.log("rx div L", rx.divWadDown(L));
...
function createDiffLowerStruct(
    ...
    int256 v,
    LogNormal.LogNormalParams memory params
) view returns (DiffLowerStruct memory) {
...
function computeLowerA(DiffLowerStruct memory params) view returns (int256) {
    int256 firstExp =
...
function computeLowerB(DiffLowerStruct memory params) view returns (int256) {
...
function computeRaiseB(DiffRaiseStruct memory params) view returns (int256) {
